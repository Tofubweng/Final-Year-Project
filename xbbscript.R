...
library(mcstate)
library(coda)
gen_sir <- odin.dust::odin_dust("odindust_xbb.R")

incidence <- read.csv("xbb15infectionsandreinfections.csv")
...
combined_compare <- function(state, observed, pars = NULL) {
  ll_xbb_infections <- ll_pois(observed$XBB.1.5.infections, state[6, , drop = TRUE])
  ll_other_infections <- ll_pois(observed$other.infections, state[10, , drop = TRUE])
  ll_xbb_reinfections <- ll_pois(observed$XBB.1.5.reinfections, state[22, , drop = TRUE])
  ...
  ll_xbb_infections + ll_other_infections + ll_xbb_reinfections + ll_other_reinfections+ll_deaths
}
