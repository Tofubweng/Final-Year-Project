...
library(mcstate)
library(coda)
gen_sir <- odin.dust::odin_dust("odindust_BA1BA2.R")

incidence <- read.csv("BA1omicroninfectionsandreinfections.csv")

dt <- 1
sir_data <- mcstate::particle_filter_data(data = incidence,time = "days",rate = 1 / dt)
rmarkdown::paged_table(sir_data)

plot(incidence$days, incidence$BA.1.infections, type = "p", xlab = "Days", ylab = "New cases")
points(incidence$week, incidence$delta_infections, col = "#cc0044")
...
combined_compare <- function(state, observed, pars = NULL) {
  ll_BA1_infections <- ll_pois(observed$BA.1.infections, state[6, , drop = TRUE])
  ll_other_infections <- ll_pois(observed$other.infections, state[10, , drop = TRUE])
  ll_BA1_reinfections <- ll_pois(observed$BA.1.reinfections, state[22, , drop = TRUE])
  ...
  ll_BA1_infections + ll_other_infections + ll_BA1_reinfections + ll_other_reinfections+ll_deaths
}
...
#Other infections plot (predominantly BA.2 especially later on)
...
# BA.1 reinfections plot

matplot(incidence$days, t(pmcmc_run$trajectories$state[22, , -1]), type = "l", xlab = "Time", ylab = "Number of individuals", ylim = c(0,3000))
matpoints(incidence$days, true_history[,14 , -1] , pch = 19, col = "#cc0044")

#Other reinfections plot

matplot(incidence$days, t(pmcmc_run$trajectories$state[18, , -1]), type = "l", xlab = "Time", ylab = "Number of individuals")
