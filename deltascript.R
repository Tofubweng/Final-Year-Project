...
library(mcstate)
library(coda)
gen_sir <- odin.dust::odin_dust("odindust_delta.R")

incidence <- read.csv("deltainfectionsandreinfections.csv")
...
combined_compare <- function(state, observed, pars = NULL) {
  ll_delta_infections <- ll_pois(observed$delta.infections, state[6, , drop = TRUE])
  ll_other_infections <- ll_pois(observed$other.infections, state[10, , drop = TRUE])
  ll_delta_reinfections <- ll_pois(observed$delta.reinfections, state[22, , drop = TRUE])
  ...
  ll_delta_infections + ll_other_infections + ll_delta_reinfections + ll_other_reinfections+ll_deaths
}
...
#Other infections plot (predominantly BA.1 especially later on)
...
# delta reinfections plot

matplot(incidence$days, t(pmcmc_run$trajectories$state[22, , -1]), type = "l", xlab = "Time", ylab = "Number of individuals", ylim = c(0,500))
matpoints(incidence$days, true_history[,14 , -1] , pch = 19, col = "#cc0044")

#Other reinfections plot

matplot(incidence$days, t(pmcmc_run$trajectories$state[18, , -1]), type = "l", xlab = "Time", ylab = "Number of individuals", ylim = c(0,3000))
